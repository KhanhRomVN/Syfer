...
/* eslint-disable */ // Syfer
import { SYFER_CTRL_K_ACTION_ID, SYFER_CTRL_L_ACTION_ID } from '../../../contrib/syfer/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	private render(): void {
		...
		const update = async () => {
			...
			// Syfer - if the workbench is empty, show open
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				openFolderButton.root.classList.add('syfer-openfolder-button')
				...
				openSSHButton.root.classList.add('syfer-openssh-button')
				...
				if (recentlyOpened.length !== 0) {
					...
					voidIconBox.append(
						...recentlyOpened.map((w, i) => {
							...
							const linkSpan = $('span');
							linkSpan.classList.add('syfer-link')
				...
				// show them Syfer keybindings
				const keys = this.keybindingService.lookupKeybinding(SYFER_CTRL_L_ACTION_ID);
				...
				const keys2 = this.keybindingService.lookupKeybinding(SYFER_CTRL_K_ACTION_ID);
