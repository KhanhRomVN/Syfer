...
import { ISyferCommandBarService } from './syferCommandBarService.js';
import { IKeybindingService } from '../../../../platform/keybinding/common/keybinding.js';
import { SYFER_ACCEPT_DIFF_ACTION_ID, SYFER_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { ISyferSettingsService } from '../common/syferSettingsService.js';
import { FeatureName } from '../common/syferSettingsTypes.js';
import { ISyferModelService } from '../common/syferModelService.js';
import { deepClone } from '../../../../base/common/objects.js';
import { acceptBg, acceptBorder, buttonFontSize, buttonTextColor, rejectBg, rejectBorder } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, SyferFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js';
// import { isMacintosh } from '../../../../base/common/platform.js';
// import { SYFER_OPEN_SETTINGS_ACTION_ID } from './syferSettingsPane.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@ISyferSettingsService private readonly _settingsService: ISyferSettingsService,
		// @IFileService private readonly _fileService: IFileService,
		@ISyferModelService private readonly _syferModelService: ISyferModelService,
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._syferModelService.initializeModel(model.uri)
	...
	// 		message: `Syfer Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'void.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open Syfer's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(SYFER_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		const { model } = this._syferModelService.getModel(uri)
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'syfer-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'syfer-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'syfer-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'syfer-greenBG', {
	...
	private _getCurrentSyferFileSnapshot = (uri: URI): SyferFileSnapshot => {
		const { model } = this._syferModelService.getModel(uri)
	...
	private _restoreSyferFileSnapshot = async (uri: URI, snapshot: SyferFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: SyferFileSnapshot = this._getCurrentSyferFileSnapshot(uri)
		let afterSnapshot: SyferFileSnapshot | null = null
		...
		const elt: IUndoRedoElement = {
			...
			undo: async () => { opts?.onWillUndo?.(); await this._restoreSyferFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreSyferFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentSyferFileSnapshot(uri)
			await this._syferModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getSyferFileSnapshot(uri: URI) {
		return this._getCurrentSyferFileSnapshot(uri)
	}


	public restoreSyferFileSnapshot(uri: URI, snapshot: SyferFileSnapshot): void {
		this._restoreSyferFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Syfer: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._syferModelService.initializeModel(uri)
		await this._syferModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Syfer: diff.type not recognized on: ${from}`)
		}
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Syfer: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`Syfer error: ${diff}.type not recognized`)
		}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@ISyferCommandBarService private readonly _syferCommandBarService: ISyferCommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(SYFER_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(SYFER_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._syferCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._syferCommandBarService.onDidChangeState(e => {
