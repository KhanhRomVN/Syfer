...
import { isFeatureNameDisabled } from '../../../../common/syferSettingsTypes.js'
...
export const useApplyStreamState = ({ applyBoxId }: { applyBoxId: string }) => {
	const accessor = useAccessor()
	const syferCommandBarService = accessor.get('ISyferCommandBarService')

	const getStreamState = useCallback(() => {
		const uri = getUriBeingApplied(applyBoxId)
		if (!uri) return 'idle-no-changes'
		return syferCommandBarService.getStreamState(uri)
	}, [syferCommandBarService, applyBoxId])
...
const tooltipPropsForApplyBlock = ({ tooltipName, color = undefined, position = 'top', offset = undefined }: { tooltipName: string, color?: IndicatorColor, position?: PlacesType, offset?: number }) => ({
	'data-tooltip-id': color === 'orange' ? `syfer-tooltip-orange` : color === 'green' ? 'syfer-tooltip-green' : 'syfer-tooltip',
...
export const useEditToolStreamState = ({ applyBoxId, uri }: { applyBoxId: string, uri: URI }) => {
	const accessor = useAccessor()
	const syferCommandBarService = accessor.get('ISyferCommandBarService')
	const [streamState, setStreamState] = useState(syferCommandBarService.getStreamState(uri))
	// listen for stream updates on this box
	useCommandBarURIListener(useCallback((uri_) => {
		const shouldUpdate = uri.fsPath === uri_.fsPath
		if (shouldUpdate) { setStreamState(syferCommandBarService.getStreamState(uri)) }
	}, [syferCommandBarService, applyBoxId, uri]))
...
const ApplyButtonsForEdit = ({
	...
	const onClickSubmit = useCallback(async () => {
		...
		if (!applyDonePromise) {
			notificationService.info(`Syfer Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`)
		}
		...
		applyDonePromise?.catch(e => {
			...
			notificationService.info(`Syfer Error: There was a problem running Apply: ${e}.`)
