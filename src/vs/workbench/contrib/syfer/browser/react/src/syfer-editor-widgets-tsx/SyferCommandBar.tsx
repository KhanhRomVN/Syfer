...
import { SyferCommandBarProps } from '../../../syferCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	SYFER_GOTO_NEXT_DIFF_ACTION_ID,
	SYFER_GOTO_PREV_DIFF_ACTION_ID,
	SYFER_GOTO_NEXT_URI_ACTION_ID,
	SYFER_GOTO_PREV_URI_ACTION_ID,
	SYFER_ACCEPT_FILE_ACTION_ID,
	SYFER_REJECT_FILE_ACTION_ID,
	SYFER_ACCEPT_ALL_DIFFS_ACTION_ID,
	SYFER_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const SyferCommandBarMain = ({ uri, editor }: SyferCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@syfer-scope ${isDark ? 'dark' : ''}`}
	>
		<SyferCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const SyferCommandBar = ({ uri, editor }: SyferCommandBarProps) => {
	...
	const syferModelService = accessor.get('ISyferModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(SYFER_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(SYFER_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(SYFER_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(SYFER_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(SYFER_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(SYFER_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(SYFER_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(SYFER_REJECT_ALL_DIFFS_ACTION_ID);
