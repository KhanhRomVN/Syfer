...
import { ProviderName, SettingName, displayInfoOfSettingName, providerNames, SyferStatefulModelInfo, customSettingNamesOfProvider, RefreshableProviderName, refreshableProviderNames, displayInfoOfProviderName, nonlocalProviderNames, localProviderNames, GlobalSettingName, featureNames, displayInfoOfFeatureName, isProviderNameDisabled, FeatureName, hasDownloadButtonsOnModelsProviderNames, subTextMdOfProviderName } from '../../../../common/syferSettingsTypes.js'
import ErrorBoundary from '../sidebar-tsx/ErrorBoundary.js'
import { SyferButtonBgDarken, SyferCustomDropdownBox, SyferInputBox2, SyferSimpleInputBox, SyferSwitch } from '../util/inputs.js'
...
const ConfirmButton = ({ children, onConfirm, className }: { children: React.ReactNode, onConfirm: () => void, className?: string }) => {
	...
	return (
		<div ref={ref} className={`inline-block`}>
			<SyferButtonBgDarken className={className} onClick={() => {
				if (!confirm) {
					setConfirm(true);
				} else {
					onConfirm();
...
const SimpleModelSettingsDialog = ({
	...
	const settingsStateService = accessor.get('ISyferSettingsService')
	...
	const sourcecodeOverridesLink = `https://github.com/voideditor/void/blob/2e5ecb291d33afbe4565921664fb7e183189c1c5/src/vs/workbench/contrib/syfer/common/modelCapabilities.ts#L146-L172`
		...
		>
			...
			>
				...
				<div className="text-sm text-void-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with Syfer, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by Syfer.`
							: `Syfer recognizes ${modelName} ("${recognizedModelName}").`}
				</div>


				{/* override toggle */}
				<div className="flex items-center gap-2 mb-4">
					<SyferSwitch size='xs' value={overrideEnabled} onChange={setOverrideEnabled} />
				...
				<div className="flex justify-end gap-2 mt-4">
					<SyferButtonBgDarken onClick={onClose} className="px-3 py-1">
						Cancel
					</SyferButtonBgDarken>
					<SyferButtonBgDarken
						onClick={onSave}
...
export const ModelDump = ({ filteredProviders }: { filteredProviders?: ProviderName[] }) => {
	...
	const modelDump: (SyferStatefulModelInfo & { providerName: ProviderName, providerEnabled: boolean })[] = []
	...
	return <div className=''>
		{modelDump.map((m, i) => {
			...
			const detailAboutModel = type === 'autodetected' ?
				<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='syfer-tooltip' data-tooltip-place='right' data-tooltip-content='Detected locally' />
				: type === 'custom' ?
					<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='syfer-tooltip' data-tooltip-place='right' data-tooltip-content='Custom model' />
					: undefined
...
const ProviderSetting = ({ providerName, settingName, subTextMd }: { providerName: ProviderName, settingName: SettingName, subTextMd: React.ReactNode }) => {
	...
	const accessor = useAccessor()
	const syferSettingsService = accessor.get('ISyferSettingsService')
	const settingsState = useSettingsState()
	...
	const handleChangeValue = useCallback((newVal: string) => {
		syferSettingsService.setSettingOfProvider(providerName, settingName, newVal)
	}, [syferSettingsService, providerName, settingName]);
...
// 				onChange={
// 					useCallback(() => {
// 						const enabledRef = syferSettingsService.state.settingsOfProvider[providerName].enabled
// 						syferSettingsService.setSettingOfProvider(providerName, 'enabled', !enabledRef)
// 					}, [syferSettingsService, providerName])}
// 				size='sm+'
// 			/> */}
// 		</div>
...
export const SyferProviderSettings = ({ providerNames }: { providerNames: ProviderName[] }) => {
	return <>
...
export const AutoDetectLocalModelsToggle = () => {
	...
	return <ButtonLeftTextRightOption
		leftButton={<SyferSwitch
			size='xxs'
			value={enabled}
			onChange={(newVal) => {
				syferSettingsService.setGlobalSetting(settingName, newVal)
...
export const AIInstructionsBox = () => {
	...
	return <SyferInputBox2
		...
		onChangeText={(newText) => {
			syferSettingsService.setGlobalSetting('aiInstructions', newText)
		}}
	/>
}
...
const FastApplyMethodDropdown = () => {
	...
	const onChangeOption = useCallback((newVal: boolean) => {
		syferSettingsService.setGlobalSetting('enableFastApply', newVal)
	}, [syferSettingsService])

	return <SyferCustomDropdownBox
		className='text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-1 rounded p-0.5 px-1'
		options={options}
		selectedOption={syferSettingsService.state.globalSettings.enableFastApply}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-void-fg-3 text-sm list-decimal select-text'>
		...
		<div
			...
			data-tooltip-id='syfer-tooltip-ollama-settings'
		>
			<ChatMarkdownRender string={`3. Run \`ollama pull your_model\` to install a model.`} chatMessageLocation={undefined} />
		</div>
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`Syfer automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	return <div
		...
		onClick={() => { syferSettingsService.setGlobalSetting('isOnboardingComplete', false) }}
	>
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	const onToggleAutoApprove = useCallback((approvalType: ToolApprovalType, newValue: boolean) => {
		syferSettingsService.setGlobalSetting('autoApprove', {
			...syferSettingsService.state.globalSettings.autoApprove,
			[approvalType]: newValue
		})
		metricsService.capture('Tool Auto-Accept Toggle', { enabled: newValue })
	}, [syferSettingsService, metricsService])
...
export const OneClickSwitchButton = ({ fromEditor = 'VS Code', className = '' }: { fromEditor?: TransferEditorType, className?: string }) => {
	...
	return <>
		<SyferButtonBgDarken className={`max-w-48 p-4 ${className}`} disabled={transferState.type !== 'done'} onClick={onClick}>
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			dataStr = JSON.stringify(syferSettingsService.state, null, 2)
			downloadName = 'syfer-settings.json'
		}
		else {
			dataStr = ''
			downloadName = ''
		...
		reader.onload = () => {
			try {
				...
				if (t === 'Chats') {
					...
					syferSettingsService.dangerousSetState(json as any)
				}
	...
	return (
		<div className={`@@syfer-scope ${isDark ? 'dark' : ''}`} style={{ height: '100%', width: '100%', overflow: 'auto' }}>
        ...
        `}
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`Syfer's Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`Syfer can access any model that you host locally. We automatically detect your local models by default.`}</h3>

									<div className='opacity-80 mb-4'>
										<OllamaSetupInstructions sayWeAutoDetect={true} />
									</div>

									<SyferProviderSettings providerNames={localProviderNames} />
								</ErrorBoundary>
							</div>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`Syfer can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>

									<SyferProviderSettings providerNames={nonlocalProviderNames} />
								</ErrorBoundary>
							</div>
								...
								<ErrorBoundary>
									...
									<div className='flex flex-col gap-y-8 my-4'>
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															<SyferSwitch
																...
																onChange={(newVal) => syferSettingsService.setGlobalSetting('enableAutocomplete', newVal)}
															/>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<SyferSwitch
															...
															onChange={(newVal) => syferSettingsService.setGlobalSetting('syncApplyToChat', newVal)}
														/>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<SyferSwitch
															...
															onChange={(newVal) => syferSettingsService.setGlobalSetting('includeToolLintErrors', newVal)}
														/>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<SyferSwitch
															...
															onChange={(newVal) => syferSettingsService.setGlobalSetting('autoAcceptLLMChanges', newVal)}
														/>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-void-fg-3 mt-1'>{`Settings that control the visibility of Syfer suggestions in the code editor.`}</div>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<SyferSwitch
															...
															onChange={(newVal) => syferSettingsService.setGlobalSetting('showInlineSuggestions', newVal)}
														/>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<SyferSwitch
															...
															onChange={(newVal) => syferSettingsService.setGlobalSetting('syncSCMToChat', newVal)}
														/>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-void-fg-3 mb-4'>{`Transfer your editor settings into Syfer.`}</h4>
								...
								<div>
									...
									<h4 className='text-void-fg-3 mb-4'>{`Transfer Syfer's settings and chats in and out of Syfer.`}</h4>
									<div className='flex flex-col gap-8'>
										{/* Settings Subcategory */}
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											<input key={2 * s} ref={fileInputSettingsRef} type='file' accept='.json' className='hidden' onChange={handleUpload('Settings')} />
											<SyferButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputSettingsRef.current?.click() }}>
												Import Settings
											</SyferButtonBgDarken>
											<SyferButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Settings')}>
												Export Settings
											</SyferButtonBgDarken>
											<ConfirmButton className='px-4 py-1 w-full' onConfirm={() => { syferSettingsService.resetState(); }}>
												Reset Settings
											</ConfirmButton>
										</div>
										...
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											...
											<SyferButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputChatsRef.current?.click() }}>
												Import Chats
											</SyferButtonBgDarken>
											<SyferButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Chats')}>
												Export Chats
											</SyferButtonBgDarken>
								...
								<div>
									...
									<ErrorBoundary>
										<div className='flex flex-col gap-2 justify-center max-w-48 w-full'>
											<SyferButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openSettings') }}>
												General Settings
											</SyferButtonBgDarken>
											<SyferButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openGlobalKeybindings') }}>
												Keyboard Settings
											</SyferButtonBgDarken>
											<SyferButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.selectTheme') }}>
												Theme Settings
											</SyferButtonBgDarken>
											<SyferButtonBgDarken className='px-4 py-1' onClick={() => { nativeHostService.showItemInFolder(environmentService.logsHome.fsPath) }}>
												Open Logs
											</SyferButtonBgDarken>
										</div>
									</ErrorBoundary>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-void-fg-3 mb-4`}>Very basic anonymous usage tracking helps us keep Syfer running smoothly. You may opt out below. Regardless of this setting, Syfer never sees your code, messages, or API keys.</h4>
								...
								`} chatMessageLocation={undefined} />
									...
									<div className='my-4'>
										<ErrorBoundary>
											<div className='flex items-center gap-x-2'>
												<SyferSwitch
													...
													onChange={(newValue) => {
														syferSettingsService.setGlobalSetting('disableSystemMessage', newValue);
													}}
												/>
										...
										<div className='text-void-fg-3 text-xs mt-1'>
											{`When disabled, Syfer will not include anything in the system message except for content you specified above.`}
										</div>
									</div>
								</div>
...
Use Model Context Protocol to provide Agent mode with more tools.
							`} chatMessageLocation={undefined} />
									...
									<div className='my-2'>
										<SyferButtonBgDarken className='px-4 py-1 w-full max-w-48' onClick={async () => { await mcpService.revealMCPConfigFile() }}>
											Add MCP Server
										</SyferButtonBgDarken>
									</div>
