...
import '../styles.css'
import { SYFER_CTRL_K_ACTION_ID, SYFER_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { Circle, MoreVertical } from 'lucide-react';
import { useEffect, useState } from 'react';

import { SyferSelectionHelperProps } from '../../../../../../contrib/syfer/browser/syferSelectionHelperWidget.js';
import { SYFER_OPEN_SETTINGS_ACTION_ID } from '../../../syferSettingsPane.js';


export const SyferSelectionHelperMain = (props: SyferSelectionHelperProps) => {
	...
	>
		<SyferSelectionHelper {...props} />
	</div>
}



const SyferSelectionHelper = ({ rerenderKey }: SyferSelectionHelperProps) => {
	...
	const ctrlLKeybind = keybindingService.lookupKeybinding(SYFER_CTRL_L_ACTION_ID)
	const ctrlKKeybind = keybindingService.lookupKeybinding(SYFER_CTRL_K_ACTION_ID)
	...
	useEffect(() => {
		const disposable = commandService.onWillExecuteCommand(e => {
			if (e.commandId === SYFER_CTRL_L_ACTION_ID || e.commandId === SYFER_CTRL_K_ACTION_ID) {
	...
	const defaultHTML = <>
		{ctrlLKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(SYFER_CTRL_L_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
		...
		{ctrlKKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(SYFER_CTRL_K_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
	...
	const moreOptionsHTML = <>
		<div
			...
			'
			onClick={() => {
				commandService.executeCommand(SYFER_OPEN_SETTINGS_ACTION_ID);
				setClickState('clickedOption');
			}}
		>
			Disable Suggestions?
