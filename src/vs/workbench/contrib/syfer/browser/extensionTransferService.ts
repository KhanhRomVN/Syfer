...
const extensionBlacklist = [
	...
	// ignore other AI copilots that could conflict with Syfer keybindings
	'sourcegraph.cody-ai',
	'continue.continue',
	'codeium.codeium',
	...
	if (os === 'mac') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Syfer', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Syfer', 'User', 'keybindings.json'),
			}, {
	...
	if (os === 'linux') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Syfer', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Syfer', 'User', 'keybindings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.vscode', 'extensions'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.void-editor', 'extensions'),
				isExtensions: true,
	...
	if (os === 'windows') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Syfer', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Syfer', 'User', 'keybindings.json'),
			}, {
