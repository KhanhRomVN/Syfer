...
import { mountSyferCommandBar } from './react/out/syfer-editor-widgets-tsx/index.js'
...
import { SYFER_ACCEPT_DIFF_ACTION_ID, SYFER_REJECT_DIFF_ACTION_ID, SYFER_GOTO_NEXT_DIFF_ACTION_ID, SYFER_GOTO_PREV_DIFF_ACTION_ID, SYFER_GOTO_NEXT_URI_ACTION_ID, SYFER_GOTO_PREV_URI_ACTION_ID, SYFER_ACCEPT_FILE_ACTION_ID, SYFER_REJECT_FILE_ACTION_ID, SYFER_ACCEPT_ALL_DIFFS_ACTION_ID, SYFER_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface ISyferCommandBarService {
...
export const ISyferCommandBarService = createDecorator<ISyferCommandBarService>('SyferCommandBarService');
...
export class SyferCommandBarService extends Disposable implements ISyferCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'void.SyferCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._syferModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(ISyferCommandBarService, SyferCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type SyferCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountSyferCommandBar(root, accessor, { uri, editor } satisfies SyferCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies SyferCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SYFER_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('voidAcceptDiffAction', 'Syfer: Accept Diff'),
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyMod.Shift | KeyCode.Enter,
				mac: { primary: KeyMod.WinCtrl | KeyMod.Alt | KeyCode.Enter },
				weight: KeybindingWeight.SyferExtension,
			}
		});
	}
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const commandBarService = accessor.get(ISyferCommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SYFER_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('voidRejectDiffAction', 'Syfer: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SYFER_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('voidGoToNextDiffAction', 'Syfer: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SYFER_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('voidGoToPrevDiffAction', 'Syfer: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SYFER_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('voidGoToNextUriAction', 'Syfer: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SYFER_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('voidGoToPrevUriAction', 'Syfer: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SYFER_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('voidAcceptFileAction', 'Syfer: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SYFER_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('voidRejectFileAction', 'Syfer: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SYFER_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('voidAcceptAllDiffsAction', 'Syfer: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SYFER_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('voidRejectAllDiffsAction', 'Syfer: Reject All Diffs in All Files'),
			keybinding: {
