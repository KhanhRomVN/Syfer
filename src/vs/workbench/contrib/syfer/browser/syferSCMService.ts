...
import { ISyferSCMService } from '../common/syferSCMTypes.js'
import { IMainProcessService } from '../../../../platform/ipc/common/mainProcessService.js'
import { ISyferSettingsService } from '../common/syferSettingsService.js'
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js'
import { ILLMMessageService } from '../common/sendLLMMessageService.js'
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/syferSettingsTypes.js'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private voidSCM: ISyferSCMService
	private loadingContextKey: IContextKey<boolean>

	constructor(
		@ISCMService private readonly scmService: ISCMService,
		@IMainProcessService mainProcessService: IMainProcessService,
		@ISyferSettingsService private readonly syferSettingsService: ISyferSettingsService,
		...
		this.voidSCM = ProxyChannel.toService<ISyferSCMService>(mainProcessService.getChannel('void-channel-scm'))
	}
	...
	async generateCommitMessage() {
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const modelSelection = this.syferSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.syferSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.syferSettingsService.state.overridesOfModel
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'SyferSCM - Commit Message' },
			})
		})
	}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCommitMessagePrompt', 'Syfer: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('voidCommitMessagePromptTooltip', 'Syfer: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCommitMessagePromptCancel', 'Syfer: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('voidCommitMessagePromptCancelTooltip', 'Syfer: Cancel Commit Message Generation'),
