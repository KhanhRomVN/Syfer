...
import { mountSyferSelectionHelper } from './react/out/syfer-editor-widgets-tsx/index.js';
...
export type SyferSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	...
	constructor(
		...
		@ISyferSettingsService private readonly _syferSettingsService: ISyferSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountSyferSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._syferSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies SyferSelectionHelperProps)
