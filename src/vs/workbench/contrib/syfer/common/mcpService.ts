...
import { MCPUserStateOfName } from './syferSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	private async _initialize() {
		try {
			await this.syferSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.syferSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.syferSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.syferSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		...
		await this.syferSettingsService.setMCPServerState(serverName, { isOn });
