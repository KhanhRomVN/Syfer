...
import { SYFER_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, SyferStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './syferSettingsTypes.js';
...
export type SyferSettingsState = {
...
// type RealSyferSettings = Exclude<keyof SyferSettingsState, '_modelOptions'>
// type EventProp<T extends RealSyferSettings = RealSyferSettings> = T extends 'globalSettings' ? [T, keyof SyferSettingsState[T]] : T | 'all'


export interface ISyferSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: SyferSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: SyferSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: SyferStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, SyferStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: SyferSettingsState): SyferSettingsState => {
...
const _validatedModelState = (state: Omit<SyferSettingsState, '_modelOptions'>): SyferSettingsState => {
...
export const ISyferSettingsService = createDecorator<ISyferSettingsService>('SyferSettingsService');
class SyferSettingsService extends Disposable implements ISyferSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: SyferSettingsState;
	...
	dangerousSetState = async (newState: SyferSettingsState) => {
	...
	private async _readState(): Promise<SyferSettingsState> {
		const encryptedState = this._storageService.get(SYFER_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(SYFER_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: SyferSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: SyferStatefulModelInfo[] = [
...
registerSingleton(ISyferSettingsService, SyferSettingsService, InstantiationType.Eager);
