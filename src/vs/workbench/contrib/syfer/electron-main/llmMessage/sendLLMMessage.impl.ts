...
import { ChatMode, displayInfoOfProviderName, ModelSelectionOptions, OverridesOfModel, ProviderName, SettingsOfProvider } from '../../common/syferSettingsTypes.js';
...
const newOpenAICompatibleSDK = async ({ settingsOfProvider, providerName, includeInPayload }: { settingsOfProvider: SettingsOfProvider, providerName: ProviderName, includeInPayload?: { [s: string]: any } }) => {
	...
	if (providerName === 'openAI') {
		...
		return new OpenAI({
			...
			defaultHeaders: {
				...
				'X-Title': 'Syfer', // Optional. Shows in rankings on openrouter.ai.
			},
			...commonPayloadOpts,
		})
	}
	...
	else throw new Error(`Syfer providerName was invalid: ${providerName}.`)
}
	...
	openai.chat.completions
		...
		.then(async response => {
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				onError({ message: 'Syfer: Response from model was empty.', fullError: null })
			}
			else {
...
const newOllamaSDK = ({ endpoint }: { endpoint: string }) => {
	...
	if (!endpoint) throw new Error(`Ollama Endpoint was empty (please enter ${defaultProviderSettings.ollama.endpoint} in Syfer if you want the default url).`)
