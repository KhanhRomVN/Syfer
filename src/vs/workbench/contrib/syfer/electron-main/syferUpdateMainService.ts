...
export class SyferMainUpdateService extends Disposable implements ISyferUpdateService {
	...
	async check(explicit: boolean): Promise<SyferCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart Syfer to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<SyferCheckUpdateRespose> {
		try {
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of Syfer is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'Syfer is up-to-date!'
					}
				}
				else {
