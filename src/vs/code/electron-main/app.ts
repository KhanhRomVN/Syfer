...
import { ISyferUpdateService } from '../../workbench/contrib/syfer/common/syferUpdateService.js';
import { MetricsMainService } from '../../workbench/contrib/syfer/electron-main/metricsMainService.js';
import { SyferMainUpdateService } from '../../workbench/contrib/syfer/electron-main/syferUpdateMainService.js';
import { LLMMessageChannel } from '../../workbench/contrib/syfer/electron-main/sendLLMMessageChannel.js';
import { SyferSCMService } from '../../workbench/contrib/syfer/electron-main/syferSCMMainService.js';
import { ISyferSCMService } from '../../workbench/contrib/syfer/common/syferSCMTypes.js';
import { MCPChannel } from '../../workbench/contrib/syfer/electron-main/mcpChannel.js';
/**
...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// Syfer main process services (required for services with a channel for comm between browser and electron-main (node))
		services.set(IMetricsService, new SyncDescriptor(MetricsMainService, undefined, false));
		services.set(ISyferUpdateService, new SyncDescriptor(SyferMainUpdateService, undefined, false));
		services.set(ISyferSCMService, new SyncDescriptor(SyferSCMService, undefined, false));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// Syfer - use loggerChannel as reference
		...
		const voidUpdatesChannel = ProxyChannel.fromService(accessor.get(ISyferUpdateService), disposables);
		...
		// Syfer added this
		const voidSCMChannel = ProxyChannel.fromService(accessor.get(ISyferSCMService), disposables);
